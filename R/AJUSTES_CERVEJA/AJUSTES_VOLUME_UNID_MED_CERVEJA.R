...
###############################################################################
  ...
  df_cerveja <- anti_join(df_cerveja,cerveja_ml, by = c("IDNFE","DET_NITEM"))

  ### AJUSTA VOLUME_SEFAZ COM ML E L CONFORME O CASO
  ## VALOR ML INTEIRO NO XPROD
  cerveja_ml$VOLUME_TRIB_AJUSTADO <- str_extract(tolower(cerveja_ml$PROD_XPROD),"(\\d{3}\\s?ml|\\d{4}\\s?ml|\\d{5}\\s?ml|\\d{6}\\s?ml|\\d\\.\\d{3}\\s?ml)")
...
#  cerveja_ml$UN_MEDIDA_SEFAZ <- ifelse(cerveja_ml$VOLUME_SEFAZ > 99,"ML","L")
  cerveja_ml$UN_MEDIDA_SEFAZ <- "ML"
  rm(id_ml)
  ########################################################

  ### SEPARA REGISTROS COM VALORES SIMILIARES A LITRO E ML EM XPROD
  id_litro <- grep("\\d{1,3}\\s?l\\s|litro|litrao",df_cerveja$PROD_XPROD,ignore.case = T)
  cerveja_litro <- df_cerveja[id_litro,]
  df_cerveja <- anti_join(df_cerveja,cerveja_litro,by = c("IDNFE","DET_NITEM"))
  ...
  cerveja_litro$VOLUME_TRIB_AJUSTADO <- cerveja_litro$VOLUME_TRIB_AJUSTADO*1000
  cerveja_litro$VOLUME_SEFAZ <- cerveja_litro$VOLUME_TRIB_AJUSTADO
  cerveja_litro$UN_MEDIDA_SEFAZ <- "ML"
#  cerveja_litro$UN_MEDIDA_SEFAZ <- ifelse(cerveja_litro$VOLUME_SEFAZ > 99,"ML","L")
  ...
  ### AJUSTA COLUNAS COM NULOS OU LIXO
  df_cerveja <- fn_ajusta_nulos(df_cerveja)
  gc(reset = T)
#######################################################################
##                          FINAL DOS AJUSTES                        ##
#######################################################################